...
# This script is a GenGM adaptation in order to generate an alignment given
...
def matrix_generation(tree, length):
    ...
    net = Phylo.to_networkx(tree) # Phylogenetic tree to graph
    ...
    # Simulated transition matrices
    real_matrices = []
    for e in edges:
        real_matrices.append(MM(e.edge[0].name, e.edge[1].name, e.transition_matrix))

    # Save the leaf sequences as an alignment in a FASTA file.
    leaves_seq = {k: v for k, v in node_sequence.items() if k.startswith('L')}
    leaves_seq = {str(k[5:]): v for k, v in leaves_seq.items()}
    leaves_seq = dict(sorted(leaves_seq.items(), key=lambda x: int(x[0])))
    ...
    return real_matrices, file_name, tree
